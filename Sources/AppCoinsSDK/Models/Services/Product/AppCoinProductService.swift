...
internal protocol AppCoinProductService {
    ...
    func getProductInformation(domain: String, currency: Currency, discountPolicy: DiscountPolicy?, result: @escaping (Result<[ProductRaw], ProductServiceError>) -> Void)
    
    func getProductInformation(domain: String, sku: String, currency: Currency, discountPolicy: DiscountPolicy?, result: @escaping (Result<ProductRaw?, ProductServiceError>) -> Void)
